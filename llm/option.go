...
type GenerateOptions struct {
	...
	StreamingFunc        func(ctx context.Context, delta *openai.ChatCompletionStreamChoiceDelta, toolResults []MCPToolExecutionResult, flag int) error `json:"-"`               // 流式响应的回调函数
	DisableStreamingFunc bool                                                                                                                           // 禁用流式输出
	...
	EnableDebug               bool            // 启用调试，主要用来打印即将发送的消息
	EnableTips                bool            // 启用每轮工具调用后添加提示词
	EnableToolsInPrompt       bool            // 启用在提示中包含可用工具列表
	EnableGuard               bool            // 启用Guard审核
	...
	DisableGuardStreaming     bool            // 禁用guard的流式输出
...
func WithStreamingFunc(streamingFunc func(ctx context.Context, delta *openai.ChatCompletionStreamChoiceDelta, toolResults []MCPToolExecutionResult, flag int) error) GenerateOption {
	return func(o *GenerateOptions) {
